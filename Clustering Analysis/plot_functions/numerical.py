...
def save_continuous_plots(algorithm_name, params, combined_profiles, save_dir):
    ...
    plt.ylabel('cluster')
    plt.savefig(
        f'{save_dir}sqm breakdown [algo<{algorithm_name}>, params<{urlencode(params)}>].png',
        bbox_inches='tight',
    )
    plt.close()
    ...
    plt.ylabel('cluster')
    plt.savefig(
        f'{save_dir}EUI breakdown [algo<{algorithm_name}>, params<{urlencode(params)}>].png',
        bbox_inches='tight',
    )
    plt.close()
    ...
    plt.ylabel('cluster')
    plt.savefig(
        f'{save_dir}entropy breakdown [algo<{algorithm_name}>, params<{urlencode(params)}>].png',
        bbox_inches='tight',
    )
    plt.close()
    ...
    plt.ylabel('cluster')
    plt.savefig(
        f'{save_dir}dayofyear breakdown [algo<{algorithm_name}>, params<{urlencode(params)}>].png',
        bbox_inches='tight',
    )
    plt.close()
    ...
    for field_to_plot in ['Sqm', 'EUI', 'entropy', 'dayofyear']:
        ...
        plt.savefig(
            save_dir
            + field_to_plot
            + f' boxplot [algo<{algorithm_name}>, params<{urlencode(params)}>].png',
            bbox_inches='tight',
        )
        plt.close()